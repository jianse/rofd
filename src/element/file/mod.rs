...
impl<'de, 'd> EMA<'de, 'd> {
    fn new(
        ...
        Self { de, ele, fields, idx: 0, value: None }
    }
}
        ...
        K: serde::de::DeserializeSeed<'de>,
    {
        ...
        let res = if let Some(Ok(attr)) = attrs.skip(self.idx).next() {
            ...
            match self.de.peek().unwrap() {
                ...
                Event::End(e) => {
                    debug_assert_eq!(self.ele.name(), e.name());
                    Ok(None)
                }
                _ => todo!()
            }
